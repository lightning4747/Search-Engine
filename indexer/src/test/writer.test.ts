...
describe('Batch Postings Writer Module', () => {
  ...
  it('should atomically write terms, postings, and update indexed_at', async () => {
    ...
    try {
      ...
      await query('DELETE FROM postings WHERE doc_id >= 992000');
      await query('DELETE FROM crawled_pages WHERE id >= 992000');
      await query('DELETE FROM urls WHERE id >= 992000');

      // Insert dummy URLs first to satisfy FK constraints
      await query(`
        INSERT INTO urls (id, url, domain, status)
        VALUES 
        (992001, 'http://test1.com', 'test1.com', 'DONE'),
        (992002, 'http://test2.com', 'test2.com', 'DONE')
      `);
      ...
      await query(`
        ...
        VALUES 
        (992001, 992001, 'Test 1', 'Desc 1', 'http://test1.com', 'Body 1', 10, 10, true, null),
        (992002, 992002, 'Test 2', 'Desc 2', 'http://test2.com', 'Body 2', 20, 20, true, null)
      `);
